...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($name) || empty($firstname) || empty($email)) {
        $insert_success = false;
        $error_message = "Problème : Veuillez remplir tous les champs requis.";  // Error message
    } else {
        // Validate email format
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            $insert_success = false;
            $error_message = "Problème : L'adresse email n'est pas valide.";  // Invalid email format
        } else {
            ...
            if ($email_check_query->num_rows > 0) {
                ...
                $insert_success = false;
                $error_message = "Problème : Cet email est déjà utilisé.";  // Duplicate email
            } else {
                ...
                if ($stmt->execute()) {
                    $insert_success = true; // Data inserted successfully
                } else {
...
<body class="relative">
    ...
    <div class="fixed inset-0 w-screen h-screen bg-cover bg-center" style="background-image: url('../src/blank-792125_1920.jpg');"></div>
    ...
    <div class="relative flex items-center justify-center min-h-screen">
        <div class="bg-white bg-opacity-90 shadow-lg mx-auto w-full max-w-[700px] p-12 rounded-2xl">
            <div class="text-center mb-8">
                <img src="../src/logo.png" alt="../src/logo du Lycée Louis Vincent" class="mx-auto mb-5 w-32 h-32">
                ...
                <?php if ($insert_success): ?>
                    ...
                    <p class="mb-5 text-base text-[#6B7280]">
                        Merci pour vos réponses ! Nous sommes ravis d’avoir pu vous accompagner lors de cet événement d’orientation. Vos retours sont précieux pour nous aider à améliorer chaque édition. Nous avons hâte de vous retrouver lors de nos prochains événements !
                    </p>
                <?php else: ?>
